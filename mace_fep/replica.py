...
class Replica:
    def __init__(
        ...
        def print_traj():
            ...
            with open(
                os.path.join(output_dir, f"thermo_traj_{self.idx}.dat"), "a"
            ) as thermo_traj:
                thermo_traj.write(
                    ("%12.1f" + " %17.6f" * 7 + "    %s" + "\n")
                    % (
        ...
        self.integrator.attach(update_nl, interval=5)
        self.integrator.attach(print_traj, interval=write_interval)

    def propagate(self, n_steps: int):

        with open(
            os.path.join(self.output_dir, f"thermo_traj_{self.idx}.dat"), "a"
        ) as thermo_traj:
            thermo_traj.write(
